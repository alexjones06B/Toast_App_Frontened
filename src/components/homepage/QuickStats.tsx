    ...
    <div className="mb-12">
      ...
      <div className="stats stats-vertical lg:stats-horizontal shadow-toast w-full bg-base-200">
        {stats.map((stat) => (
          <div
            key={stat.label}
            className="stat bg-base-200 hover:bg-base-300 transition-all hover:-translate-y-1 hover:shadow-toast-hover"
          >
            <div
              className={`stat-value text-neutral ${typeof stat.value === "string" ? "text-4xl" : ""}`}
            >
              {stat.value}
            </div>
